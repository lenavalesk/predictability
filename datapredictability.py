# -*- coding: utf-8 -*-
"""DataPredictability.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1UIOccxmQPl2O8Agk6qpccPmwQN_eJFDv
...
# Monto el drive para los archivos
...
oracFileName = "/content/drive/My Drive/Cuarentesis/textos.csv"
...
modiFileName = "/content/drive/My Drive/Cuarentesis/modificaciones.csv"
columns               = ['palNumSt','st']
...
path = "/content/drive/My Drive/Cuarentesis/log.csv"
...
log = pd.read_csv(path, encoding='iso-8859-1',header=None, names=columnas, quotechar="'")
print(log)
log = pd.read_csv(path, encoding='iso-8859-1',header=None, names=columnas, quotechar="'")
...
print(log)
print(log.columns)
#Esta mas limpio pero sigue teniendo cosas que no nos interesan
...
# respuestas.to_csv('respuestas.csv')
# respuestas.csv "drive/My Drive/Cuarentesis"
...
stories_full = pd.read_csv("/content/drive/My Drive/Cuarentesis/predictability_full.csv", index_col=0)
stories_full
...
humanFileName = "/content/drive/My Drive/Cuarentesis/mysavefile.csv"
data_human = pd.read_csv(humanFileName, encoding='iso-8859-1')

log_human = data_human[['freq', 'wordid']].copy()   #El text id no me sirve, esta raro
#log_human.drop_duplicates(inplace=True)
#log_human
#log_human = log_human.rename(columns={'freq':'freq', 'wordid':'palabra'})
#log_human['palabra'] = [unidecode.unidecode(x.lower().strip()) for x in log_human['palabra']]
#log_human = log_human.drop_duplicates()



#log_human.loc[log_human['palabra'] == 'que']
#boolean = log_human['palabra'].duplicated().any()
#log_human[log_human[['palabra']].duplicated() == True]

#a = pd.DataFrame(log_human.groupby("palabra"))

# log_human.to_csv('loghuman.csv')
# loghuman.csv "drive/My Drive/Cuarentesis"

# log_std = log_human.groupby("wordid").std().loc[log_human.groupby("wordid").std()["freq"]!=0]
# log_std.to_csv('log_std.csv')
# log_std.csv "drive/My Drive/Cuarentesis"

#full_freq = pd.merge(log_human, log_full, on=['palabra'], how='left')
#full_freq.drop_duplicates(inplace=True)
#full_freq

#orac_freq = pd.merge(log_human, pred, on=['palabra'], how='left')
#orac_freq.drop_duplicates(inplace=True)
#orac_freq

full_orac_freq = pd.merge(full_freq, orac_freq, left_on=['text_id', 'palNum'], right_on=['text_id', 'palNumGlobal'])
full_orac_freq

# ########Agrupo por freq de palabra en lexico############

groupped_freq = (
     full_orac_freq.groupby('freq_x').agg({'pred_x': ['mean', 'count', 'std'], 'pred_y': ['mean', 'count', 'std'], 'entropy_x': ['mean', 'count', 'std'], 'entropy_y': ['mean', 'count', 'std'], 'surprisal_x': ['mean', 'count', 'std'], 'surprisal_y': ['mean', 'count', 'std']})).dropna()

groupped_freq['error_pred_x'] = (groupped_freq['pred_x']['std']) / np.sqrt(groupped_freq['pred_x']['count'])
groupped_freq['error_pred_y'] = (groupped_freq['pred_y']['std']) / np.sqrt(groupped_freq['pred_y']['count'])

groupped_freq['error_entropy_x'] = (groupped_freq['entropy_x']['std']) / np.sqrt(groupped_freq['entropy_x']['count'])
groupped_freq['error_entropy_y'] = (groupped_freq['entropy_y']['std']) / np.sqrt(groupped_freq['entropy_y']['count'])

groupped_freq['error_surprisal_x'] = (groupped_freq['surprisal_x']['std']) / np.sqrt(groupped_freq['surprisal_x']['count'])
groupped_freq['error_surprisal_y'] = (groupped_freq['surprisal_y']['std']) / np.sqrt(groupped_freq['surprisal_y']['count'])
...
plt.errorbar(groupped_freq.index + 0.1, groupped_freq['entropy_x']['mean'], yerr=groupped_freq['error_entropy_x'],linewidth=1, label='Contextualizada')
plt.errorbar(groupped_freq.index, groupped_freq['entropy_y']['mean'], yerr=groupped_freq['error_entropy_y'],linewidth=3, label='Oraciones Aisladas')


#plt.title(label="Stories")
plt.xlabel(xlabel='Freq', fontsize=size_letters)
plt.ylabel(ylabel='Entropy', fontsize=size_letters)
plt.xticks(fontsize=size_letters)
plt.yticks(fontsize=size_letters)
